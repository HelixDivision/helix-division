...
- **Textarea / TextareaField** (`ui/textarea.tsx`, `ui/textarea-field.tsx`, Phase 9) — multiline siblings of `input.tsx`/`text-field.tsx`, same border/focus/`aria-invalid` treatment and the same react-hook-form `Field`/label/error wiring. First needed by the admin product/category forms.
- **Admin surfaces** (`app/(admin)/admin/*` + `components/admin/*`, Phase 9) — the admin shell reuses the same sidebar/`SignOutButton` pattern as the account area. New shared admin primitives: `StatCard` (dashboard tiles, `default`/`warning` tone), `AdminToolbar` (URL-driven search + filter selects, debounced — the ShopFilters pattern generalized), and `AdminPagination` (prev/next preserving other searchParams). Tables use the existing `ui/table.tsx` inside an `overflow-x-auto` wrapper (horizontal scroll on mobile rather than reflow). Everything else (product/category/stock/order dialogs and forms) composes existing `Dialog`/`Field`/`Select`/`Checkbox`/`DropdownMenu`/`Button` primitives — no new tokens. `OrderStatusBadge`/`OrderSummaryCard`/`ShippingAddressCard` (Phase 8, `components/checkout/`) are reused across the admin order views too.
