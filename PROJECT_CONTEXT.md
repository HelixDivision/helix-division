...
- **Git**: initialized during the Phase 4→5 transition (no repo existed before that). Checkpoint commits so far: `feat(shop): complete Phase 4 Shop Experience`, `docs: finalize Phase 5 architecture and implementation`, a Real Prisma Integration checkpoint, a UX polish pass checkpoint, a Phase 7 Authentication checkpoint, a Phase 8 Customer Accounts checkpoint, a pre-Phase-9 architecture-review checkpoint, and a Phase 9 Admin Dashboard checkpoint — check `git log` to confirm which commit you're resuming from.
- **Not started**: CMS content wiring (`Page`/`Article`/`FAQItem` are still hardcoded in home components, not read from the DB — next up, Phase 10), real integrations for NOW Payments/Coinbase Commerce/Wise (adapters are scaffolded, not implemented), the admin v2 modules (Coupons/Discounts/Reviews/Shipping/Returns/Analytics/Content/Email/Media/Users&Roles — models exist, no UI), binary image upload (needs a storage provider), plus everything AUTH.md documents as a future extension point (real OAuth provider, real MFA, real rate limiter, real audit sink). **See `ROADMAP.md` for the full remaining plan and phase order.**
...
│   ├── components/                ui/ (primitives, incl. textarea/textarea-field added Phase 9) · layout/ · home/ · shop/ · cart/ · checkout/ (incl. OrderSummaryCard/ShippingAddressCard/OrderStatusBadge, shared with account+admin) · account/ · admin/ (AdminNav, StatCard, AdminToolbar, AdminPagination, ProductForm, ProductImagesForm, ProductRowActions, CategoryManager, StockAdjustDialog, OrderStatusControls) · motion/
...
│   ├── server/
│   │   ├── actions/                Server Actions — checkout.ts, catalog.ts, auth.ts, account.ts, shared.ts (ActionResult/errorMessage/
│   │   │                            fieldErrorsFrom/requireAdmin — sync helpers, NOT "use server"), admin-{products,categories,inventory,orders}.ts
│   │   ├── services/                Business logic: catalog.ts (re-exports lib/catalog.ts), orders.ts (orchestrator, incl. admin transitions),
│   │   │                            shipping.ts, tax.ts, discounts.ts, inventory.ts (real — PrismaInventoryService), notifications.ts,
│   │   │                            auth.ts (auth + changePassword), user.ts (profile/addresses), auth-audit.ts, rate-limit.ts,
│   │   │                            admin-{products,categories,inventory,customers,dashboard}.ts (Phase 9 admin read/write)
...
| `/admin`, `/admin/products`(+`/new`,`/[id]`), `/admin/categories`, `/admin/inventory`, `/admin/orders`(+`/[id]`), `/admin/customers`(+`/[id]`) | **Built** — Phase 9 (role-gated ADMIN). v2 admin modules still model-only, no UI |
...
1. Read this file, then `ROADMAP.md` to see what phase comes next (Phase 10 — CMS / Content Layer, as of this writing), then skim `ARCHITECTURE.md`/`DESIGN_SYSTEM.md`/`API.md`/`AUTH.md` for whatever the task touches.
