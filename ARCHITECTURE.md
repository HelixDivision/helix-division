...
**Status**: Phases 1–9 complete, plus the Phase 9.5 admin/content expansion (Media Library, COA uploads, Research Center CMS, Newsletter CMS, Analytics), plus a Prototype Launch marketing sprint (About/Quality/Contact/FAQ/Legal + SEO/error boundaries). The generated Prisma client is now committed to the repo for Vercel deployment (see [§ Build Phasing](#build-phasing)). A real Postgres database (hosted on Neon) is the single source of truth for the catalog and orders — see [§ Data & Persistence](#data--persistence-a-real-database). Inventory is really tracked (Phase 9): `InventoryService` decrements stock through the order lifecycle and out-of-stock items are blocked at checkout server-side. File uploads go through a swappable `StorageProvider` (Phase 9.5, local-FS today) — see [§ Storage Architecture](#storage-architecture). Next up is real payment-provider integrations / production hardening; see [ROADMAP.md](./ROADMAP.md).
...
├── src/
│   ├── app/
│   │   ├── (marketing)/{page.tsx (home), about/, quality/, contact/, faq/, research/, newsletter/, legal/{terms,privacy,shipping,research-disclaimer}}   — all built (home Phase 3; research/newsletter Phase 9.5; about/quality/contact/faq/legal Prototype Launch)
│   │   ├── (shop)/
...
- **`notifications.ts`** — `NotificationService` (order confirmation/new-order-alert, payment received, shipment, email verification, password reset, contact, newsletter). **`ResendNotificationService` is the live implementation** — real transactional email via Resend (see [§ Email & Notifications](#email--notifications)); templates in `lib/email/templates.ts`, SDK wrapper in `lib/email/client.ts`. Wired: `sendOrderConfirmation` after order creation (customer confirmation + internal new-order alert to `SUPPORT_EMAIL`), `sendPaymentReceived` after "I've sent the transfer," `sendContactMessage`/`sendNewsletterConfirmation` from their services, and the auth flows.
...
No `/peptides`-specific routing or schema. `Category` rows carry an `attributeSchema` describing which attribute keys/labels their products expose; `ProductVariant` carries a matching `attributes: Json` blob (e.g. `{ "dosage": "10mg" }` for a peptide, `{ "size": "L", "color": "black" }` for merch). The PDP template (`[category]/[slug]/page.tsx`) and catalog filters read `attributeSchema` to render the right fields generically — a new category is a data insert, never a code change. **Pricing lives on `ProductVariant.price` (a nullable `Decimal(10,2)`), not on `Product`** (`Product` has no price column) — a variant with `price: null` renders "Contact for Pricing" or "Coming Soon" instead of an invented figure; this is intentional, not a gap to fill in with placeholder numbers. `compareAtPrice` (also nullable) sits alongside it on the variant.
...
File uploads (Phase 9.5) go through a two-method `StorageProvider` interface (`src/lib/storage/`), the same swappable-adapter pattern as payments/notifications. **Two adapters exist and `provider.ts` selects by env**: `VercelBlobStorageProvider` (`@vercel/blob`) when `BLOB_READ_WRITE_TOKEN` is set — the production backend — otherwise `LocalStorageProvider` (writes `public/uploads/<folder>/`, served statically, returns `/uploads/...` URLs) for dev. No call site knows which is live; they hand over bytes and get back a public URL.

**Why the split**: **Vercel's serverless filesystem is read-only** (except `/tmp`), so `LocalStorageProvider`'s `mkdir`/`writeFile` under `public/uploads` throw `EROFS` in production — the original cause of "media upload doesn't complete" on Vercel. `VercelBlobStorageProvider` uploads to Vercel Blob and returns absolute `https://…public.blob.vercel-storage.com/…` URLs (rendered via `next/image`; remote hosts allowed in `next.config.ts`). Local dev keeps zero-credential local-FS storage. Enabling it in production = connect a Blob store in Vercel (injects `BLOB_READ_WRITE_TOKEN`) + redeploy; no code change. `public/uploads/` is gitignored (only a `.gitkeep` is tracked).
...
Transactional email is delivered through the same swappable-interface pattern as payments/storage. `server/services/notifications.ts` defines `NotificationService`; **`ResendNotificationService` is the live implementation** (it replaced the earlier `ConsoleNotificationService` stub). No caller imports Resend or a template directly — they call the service, preserving the boundary.

- **SDK wrapper** (`lib/email/client.ts`): lazily constructs the Resend client from `RESEND_API_KEY` (env only — never hardcoded). `EMAIL_FROM` (default `Helix Division <support@helixdivision.com>`) is the sender; `SUPPORT_EMAIL` (default `support@helixdivision.com`) is where internal/staff notifications go. `sendEmail()` is **best-effort**: with no key it logs (local dev), and any send failure is caught + logged, **never thrown** — so checkout, contact, newsletter signup, registration, and password reset can't break on an email failure.
- **Templates** (`lib/email/templates.ts`): pure functions → `{ subject, html, text }`. Every email is multipart (responsive inline-styled HTML + a plain-text alternative), Helix-branded, no external assets. All caller-supplied text is HTML-escaped.
- **What sends, and to whom**:
  - New order → customer confirmation **and** an internal new-order alert to `SUPPORT_EMAIL` (order #, customer, email, payment method, status, items/qty/prices, totals, timestamp, shipping address). Both fire from `sendOrderConfirmation` (the chosen `providerId` is threaded through so the payment method is known before a `Payment` row exists).
  - Contact submission → internal alert to the configured recipient (reply-to the sender, includes name/email/subject/message/date; IP/UA only if captured — not captured today) **and** a confirmation to the submitter.
  - Newsletter subscribe → branded welcome to the subscriber **and** a new-subscriber alert to `SUPPORT_EMAIL`.
  - Password reset, email verification → the user. Payment received, shipment → the customer.
...
- **Prototype Launch sprint** (not a numbered phase) — the remaining mockup-driven marketing pages (**About** `/about`, **Quality** `/quality`, **Contact** `/contact` with a functional, spam-protected, DB-persisted contact form → Admin → Messages, admin-configurable recipient at Admin → Settings), **FAQ** and **Legal** pages, plus production-readiness (dynamic `sitemap.ts`/`robots.ts`, generated `opengraph-image`, `not-found`/`error`/`global-error` boundaries). Added the `ContactMessage`/`SiteSetting` models. See [DEPLOYMENT.md](./DEPLOYMENT.md).
- **Deployment prep** (current `HEAD`, `Fix Prisma client for Vercel deployment`) — the generated Prisma client (`src/generated/prisma/`) was **committed to the repo** (removed from `.gitignore`) so a Vercel build doesn't have to regenerate it; there is no `postinstall: prisma generate`. Trade-off: after any schema change the client must be regenerated **and re-committed** or it drifts from the schema. No application logic changed.
